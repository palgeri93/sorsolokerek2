...
def _safe_float(x, default: float = 1.0) -> float:
    """Convert Excel/Streamlit values to float safely (handles NaN/None/str)."""
    try:
        if x is None:
...
if (chosen not in st.session_state["weight_overrides_by_sheet"]) or (
    set(st.session_state["weight_overrides_by_sheet"][chosen].keys()) != set(names_all)
):
    # Inicializálás az Excelből beolvasott "Súly" oszlop alapján
    st.session_state["weight_overrides_by_sheet"][chosen] = {
        n: _safe_float(w, 1.0) for n, w in zip(names_all, weights_all)
    }

# A ténylegesen használt súlylista (mindig a felülírt érték)
weights_effective_all = [
    _safe_float(st.session_state["weight_overrides_by_sheet"][chosen].get(n, 1.0), 1.0)
    for n in names_all
]
